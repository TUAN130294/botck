...
# -*- coding: utf-8 -*-
"""
Train TOP 62 Liquidity Stocks - PARALLEL GPU VERSION
====================================================
...
def train_single_stock(symbol, index, total):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"[{index}/{total}] ✓ {symbol} DONE - Val loss: {val_loss:.6f if val_loss else 'N/A'} ({elapsed:.1f}s)")
            ...
            print(f"[{index}/{total}] ✗ {symbol} FAILED ({elapsed:.1f}s)")
        ...
        print(f"[{index}/{total}] ⏱ {symbol} TIMEOUT (>10 min)")
        return {
            'symbol': symbol,
            'status': 'TIMEOUT',
            'val_loss': None,
        ...
        print(f"[{index}/{total}] ✗ {symbol} ERROR: {str(e)}")
        return {
            'symbol': symbol,
            'status': 'ERROR',
            'val_loss': None,
...
def train_top62_parallel(max_workers=4):
    """Train all TOP 62 stocks in parallel
    ...
    print("="*80)
    print("TRAINING TOP 62 LIQUIDITY STOCKS - PARALLEL GPU MODE")
    print("="*80)
    ...
    output_file = f"training_results_top62_parallel_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Train TOP 62 stocks in parallel')
    ...
    results = train_top62_parallel(max_workers=max_workers)
